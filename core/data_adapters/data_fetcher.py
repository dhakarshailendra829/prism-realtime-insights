...
# FLEXIBLE CONFIG - code khud column names find karega
FILE_CONFIG = {
    "Server Metrics (Predictive)": {
        "path": "data/Attack_Dataset.csv",
        "possible_keys": ["Scenario Description", "Description", "Title", "Scenario"],  # Multiple options
        "unit": "% Attack Load"
    },
    "IoT Sensors (Anomaly Detection)": {
        "path": "data/HR_Analytics.csv",
        "possible_keys": ["MonthlyIncome", "Monthly Income", "Income"],  # Auto-detect
        "possible_attrition": ["Attrition", "attrition", "Left"],  # Auto-detect
        "unit": "Monthly Income (USD)"
    },
    "Social Media (Text Analysis)": {
        "path": "data/Sentiment_Data.csv",
        "possible_keys": ["statement", "Statement", "text", "Text", "comment", "Comment"],  # Auto-find text column
        "unit": "Text Statement"
    },
}
...
def auto_detect_column(df: pd.DataFrame, possible_keys: list) -> str:
    """Auto-detect best matching column name"""
...
def fetch_real_or_simulated_data(domain: str) -> Dict[str, Any]:
    ...
    # Sequential streaming with loop-back
    ...
    if domain == "Social Media (Text Analysis)":
        ...
        # Generate load % from text length
        text_len = len(str(row[key_col]))
        load_percent = 40 + min(text_len / 10, 59)  # Scale based on description length
        return {
            'type': 'numeric_series',
            'value': load_percent,
            'unit': unit,
